...
import com.palash.onedrive4j.OneDriveFileSystem;
import com.palash.onedrive4j.OneDriveFileSystemProvider;
import com.palash.onedrive4j.OneDrivePath;
...
public class OneDrive {
    ...
    public List<Drive> getDrives() throws IOException {
        return getDriveService().getDrives(_accessToken);
    }

    public List<Path> getRootPath() throws IOException {
        List<DriveItem> rootItems = getDriveService().getRootItems(_accessToken, getDrives().get(0).id);
        List<Path> rootPathItems = new ArrayList<>();
        for (DriveItem rootItem : rootItems) {
            rootPathItems.add(new OneDrivePath(getFileSystem(), rootItem));
        }
        return rootPathItems;
    }

    public List<Path> getRootPath(String driveId) throws IOException {
        List<DriveItem> rootItems = getDriveService().getRootItems(_accessToken, driveId);
        List<Path> rootPathItems = new ArrayList<>();
        for (DriveItem rootItem : rootItems) {
            rootPathItems.add(new OneDrivePath(getFileSystem(), rootItem));
        }
    ...
    OneDriveFileSystem oneDriveFileSystem = null;

    public FileSystem getFileSystem() {
        if (oneDriveFileSystem == null) {
            oneDriveFileSystem = new OneDriveFileSystem(new OneDriveFileSystemProvider(this));
        }
        return oneDriveFileSystem;
    }
}
